...
def norm_pi_2_neg_pi(ang):
    ...
    if ang<=-math.pi:
        return ang + (2 * math.pi)
    return ang
#matrics for orinatation update and kalman
def A(dt=const_dt):
...
def R_moving(mag=False):
    if mag==False:
        return np.array([ [1000000, 0, 0]
                        , [0, 1000000, 0]
                        , [0, 0, 0]])
    else:
        return np.array([ [100000, 0, 0]
                        , [0,100000, 0]
                        , [0, 0,100000]])
...
def orantaion_update(gyr_vec,X_n,P_n,dt=const_dt):
    ...
    # perdict case 1
    X_hat_n_plus1 = A() @ X_n + 0.9905*B(dt) @ E_R(X_n[0, 0], X_n[1, 0]) @ gyr_vec #np.array([[1],[1],[0.9905]])
...
def kalman(acc_vec,X_n,P_n,R_func,dt=const_dt):
    ...
    #print(np.array([[roll_g_321], [pitch_g_321], [0]])  - X_n)
    X_n_plus1 = X_n + K(P_n, C(), R_func()) @ (np.array([[roll_g_321], [pitch_g_321], [0]]) - C() @ X_n)
    P_n_plus1 = P_n - K(P_n, C(), R_func()) @ C() @ P_n

    return (X_n_plus1, P_n_plus1)

def kalman_roll(acc_vec,X_n,P_n,dt=const_dt):
    roll_g_321 = roll_from_g_321(acc_vec[0, 0], acc_vec[1, 0], acc_vec[2, 0])

    #print(np.array([[roll_g_321], [pitch_g_321], [0]])  - X_n)
    X_n_plus1 = X_n + K(P_n, C_roll(), R_roll()) @ (np.array([[roll_g_321], [0], [0]]) - C_roll() @ X_n)
    P_n_plus1 = P_n - K(P_n, C_roll(), R_roll()) @ C_roll() @ P_n
...
def acc_earth(X_n,acc_vec):
    ...
    if time.time()-last_t>5:
        print((Rot(X_n[0, 0], X_n[1, 0], X_n[2, 0]).T @ acc_vec) + np.array([[0], [0], [const.g]]))
        last_t=time.time()
...
def ondata():
    ...
    while True:
        ...
        if sensorType == "android.sensor.accelerometer" and calib_with_g <= 0:
            ...
            if strong_calib_key == 1:
                temp_x_n = X_n
                X_n, P_n = kalman(acc_vec_g, X_n, P_n, R_not_moving, dt=const_dt)
                strong_calib_key=0
                print('--------------roll pitch correction------------')
                print(X_n - temp_x_n)

            if end_of_step == True:
                if (time.time_ns()-time_from_end)>23000000 and (time.time_ns()-time_from_end)<40000000:
                    X_n, P_n =kalman(acc_vec_g, X_n, P_n, R_moving, dt=const_dt)
                    #X_n, P_n = kalman_roll(acc_vec_g, X_n, P_n, dt=const_dt)
            ...
            # print(f"after_num_non_move:{curr_and_after_num_non_move} before_num_non_move:{before_num_non_move}")
            ...
            if len(acc_delay_que) == non_move_after + 1:
                ...
                if before_num_non_move + curr_and_after_num_non_move == non_move_before + 1 + non_move_after:
                    if end_of_step == False:
                        ...
                        acc_for_calc = deque()
                        acc_dt = deque()
                        # max(before_num_non_move + 1, non_move_before)
        ...
        if sensorType == "android.sensor.gyroscope" and calib_ang <= 0:
            ...
            x, y, z = values
            gyr_vec_no_off =np.array([[y], [-z], [-x]])
            gyr_vec = gyr_vec_no_off -((1/1200)*offset)

            X_n, P_n = orantaion_update(gyr_vec, X_n, P_n, (timestamp - last_gyr) * 1e-9)
            last_gyr = timestamp
            if time.time()-last_t2>1:
                print(X_n.flatten())
                last_t2=time.time()
            if mid_calib>0:
                offset2=offset2 + gyr_vec_no_off
                mid_calib=mid_calib-1
                if mid_calib==0:
                    offset=offset2
                    print("end mid calib",offset.flatten())
        ...
        try:
            ...
            for s in ready:
                ...
                print("--------end------------")
                print("x_n", X_n)
                X_n[2,0]=X_n[2,0]+(1*norm_pi_2_neg_pi((est_vec[2]-X_n[2,0])))
